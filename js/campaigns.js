...
async function toggleCampaignSubtask(parent,gid){const arr=state.campaignSubtasks[parent]||[],sub=arr.find(x=>x.gid===gid);if(!sub)return;sub.completed=!sub.completed;renderCampaigns();try{await call("update_tasks",{tasks:[{task:gid,completed:sub.completed}]});if(sub.completed)celebrateCompletion({name:sub.name,isSubtask:true},{compact:true});else toast("Back in play");}catch(e){sub.completed=!sub.completed;renderCampaigns();toast("Failed: "+e.message);}}
