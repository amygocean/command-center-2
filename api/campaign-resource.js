...
async function analyseWithAI({buf,mime,name,category,campaign,text}){
  ...
  const schema=`Return ONLY valid JSON with this exact shape: {"summary":"", "facts":[""], "recipes":[{"name":"","prepared_by":"","important_steps":[""],"ingredients_or_products":[""],"pop_ups":[""],"suggested_shots":[""]}], "dates":[{"date":"","meaning":""}], "audiences":[""], "risks":[""], "gaps":[""], "shoot_ideas":[{"title":"","why":"","shots":[""]}], "output_ideas":[{"title":"","type":"course|skills_booster|manager_support|community_message|infographic|cheat_sheet|video|photo|other","audience":[""],"why":""}]}. Do not invent operational facts. Put uncertainty or conflicting information in gaps.`;
  const context=`You are analysing source material for an Ocean Basket Academy learning campaign. Campaign: ${campaign?.name||"Untitled"}. Launch date: ${campaign?.launch||campaign?.start||"not set"}. Resource category: ${category||"Other"}. File: ${name}. These are usually Ocean Basket recipe cards. For EACH recipe capture: name; prepared_by = the station exactly as written (e.g. "Grill Line", "Sushi"); ingredients_or_products = each ingredient WITH its exact quantity as written (e.g. "4pc Rock Shrimp Tempura", "30ml Spicy sauce", "1 Lemon wedge"); important_steps = the method/plating steps in order; pop_ups = short exact on-screen subtitle wording a training video should display (e.g. "1 Lemon wedge", "200g chips — bottom-left of box"); suggested_shots = the camera shots to capture. Also identify affected audiences, shoot needs and missing information. ${schema}`;
  let out="";
  if(openaiKey){
